...
- [`marin-digital-standards/security`](https://github.com/marincountygov/marin-digital-standards/tree/main/security) and [`marin-os/schemas/security.schema.json`](https://github.com/marincountygov/marin-os/blob/main/schemas/security.schema.json) — only if the app isn't a typical internal staff tool (see `security/profiles.md` for `public-web`/`public-api`/`authenticated`/`custom`); the template's starter `security.json` already covers the default `internal` case.
...
2. Start the project from `marin-app-template` (use it as the GitHub template, or copy its structure) rather than assembling `index.html`, the shell, branding, menu behavior, or font loading from scratch. Its vendored `marin-ui` bundle already provides Pico.css, `app-brand.css`, `app-shell.js`, local Open Sans for body/UI text, and local Jost for heading/display text. Don't introduce React, Vue, Svelte, Angular, Next.js, Tailwind, Bootstrap, npm build steps, frontend routing frameworks, Google Fonts, Adobe Fonts, jsDelivr, unpkg, cdnjs, or other runtime static-asset CDNs unless explicitly requested and justified — the template deliberately has none.
3. Replace the `APP_NAME`, `APP_DESCRIPTION`, `APP_OWNER`, and `APP_REPO` placeholders in `index.html`, `README.md`, and `security.json` (the `exceptions[].owner` and `review.owner` fields) with the real product name, one-sentence plain-language description, owner, and GitHub repo slug.
4. Add only application-specific code: replace the starter `#start` section in `index.html` with the real workflow, add app-specific state and behavior to `assets/app.js`, and add app-specific styling to `assets/app.css` only after confirming an existing `marin-ui` component or token doesn't already cover it. Do not add app-level `@import` font rules, Google Fonts links, external CDN CSS/JS includes, or ad hoc `font-family` stacks; inherit the template's `--app-font-body` and heading tokens unless a standard explicitly says otherwise. Leave `#about` and `#updates` in place — every app keeps About and Updates in `#app-nav`; `#start` already covers the "Home" case as the default view's own task tab, so no separate Home link is needed unless the workflow grows a distinct landing view later (see `marin-ui/docs/components.md`, "Standard app nav"). The default view must stay immediately functional — keep info/how-to/context content out of it and in About instead.
...
7. Pick the app's icon before registering it: choose the closest stock Lucide icon to what the app does (see `marin-ui/docs/components.md`, "App icon," and `marin-ui/vendor/icons/lucide/` for what's already vendored — add the one new SVG file there if the app needs an icon not yet in that folder, sourced from lucide.dev, ISC-licensed). Use that single icon in all three places: the template's placeholder header `.app-icon`, the placeholder favicon `data:image/svg+xml` URI (same rounded-square/gold-stroke wrapper, just the app-specific path data), and the `catalog.json` `icon` field below. Never leave the template's generic four-square placeholder in a shipped app.
8. If the app is meant to be part of MarinOS (not a one-off/private tool), register it: add an entry to [`marin-os/catalog.json`](https://github.com/marincountygov/marin-os) (id, name, type, url, description, audience, status, owner, `icon` — the same icon from step 7) and the matching directory card to `marin-os/index.html`, then run `node scripts/check-catalog-sync.js` in that repo. That's the entire registration step — every other MarinOS app's banner menu picks up the new entry automatically at runtime; don't hand-edit any other app's repo. Also create a `marin.yml` manifest in the new app's repo (see `marin-app-template/marin.yml` for the fields and an existing app's for a filled-out example) and a short `AGENTS.md` pointing at `marin-ui`, `marin-digital-standards`, and `marin-skills`.
9. Confirm the security setup the template already ships with actually fits this app rather than leaving it on autopilot: is `internal` (the template's default) still the right profile, or does this app need `public-web`/`public-api`/`authenticated`/`custom` per `marin-digital-standards/security/profiles.md`? Does `data.acceptsUserSubmittedContent`/`collectsSensitiveData`/`externalDataSources` in `security.json` still reflect what this app actually does, now that step 4's real workflow exists? Run `node marin-os/scripts/validate-security.js security.json` and `node marin-os/scripts/check-security.js .` (paths relative to a local `marin-os` checkout) before shipping. Route a deeper pass — CSP specifics, a genuine exception, anything beyond confirming the defaults still fit — to `security-review`.
...
No runtime font or static UI asset is loaded from Google Fonts, Adobe Fonts, jsDelivr, unpkg, cdnjs, or a similar external CDN.
The app has a skip link and one main landmark.
Open Sans is used for body and user-interface text from `vendor/fonts/open-sans/OpenSans-VariableFont_wdth,wght.woff2`, with `vendor/fonts/open-sans/OFL.txt` present.
Jost is used for heading/display text from a local bundled font file with accessible fallbacks.
The app has one distinctive icon (from `vendor/icons/lucide/`, not the template's generic four-square placeholder), used identically in the header `.app-icon`, the favicon, and `marin-os/catalog.json` — not a generic icon in some of those and a distinctive one in others.
...
security.json's profile, data declarations, and owner fields reflect this app specifically (not left as the template's generic internal-tool defaults if this app is actually public-facing, authenticated, or handles sensitive data) — see `marin-digital-standards/security/profiles.md`.
`node marin-os/scripts/check-security.js .` passes (security.json, SECURITY.md, README security section, #security tab, About link, security.txt all present and consistent).
```
...
This skill builds and reviews the *whole* app shell — it doesn't replace a focused review. For a deep pass on one dimension, use the specific skill instead: `accessibility-review` for WCAG conformance beyond this checklist, `design-review` for County visual-identity compliance, `plain-language-review` or `inclusive-language-review` for content, `digital-service-design` for a resident-facing service's task flow, `security-review` for CSP specifics, a genuine security exception, or anything beyond confirming the template's security defaults still fit.
